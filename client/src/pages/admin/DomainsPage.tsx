...
export default function DomainsPage() {
  ...
  const expiringSoon = domains.filter((d) => daysUntil(d.expiration_date) <= 30).length;
  ...
  const openEdit = (d: Domain) => {
    setEditing(d);
    setForm({ client_id: d.client_id, domain: d.domain, registrar: d.registrar, expiration_date: d.expiration_date, auto_renew: d.auto_renew });
    setModalOpen(true);
  };
        ...
        {expiringSoon > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <p className="text-amber-300 text-sm">
              <strong>{expiringSoon} domínio{expiringSoon > 1 ? "s" : ""}</strong> expira{expiringSoon === 1 ? "" : "m"} em menos de 30 dias.
            </p>
          </motion.div>
        )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-purple-900/20">
                  {["Domínio", "Cliente", "Registrar", "Vencimento", "Auto-renovar", ""].map((h) => (
                  ...
                  : domains.map((d) => {
                      ...
                      return (
                        <motion.tr key={d.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                          className={cn("hover:bg-white/[0.02] transition-colors group",
                            expired && "bg-red-500/[0.04]",
                            !expired && expiring && "bg-amber-500/[0.03]")}>
                          <td className="px-5 py-3.5 text-gray-200 font-medium">{d.domain}</td>
                          <td className="px-5 py-3.5 text-gray-400">{clientMap[d.client_id] ?? "—"}</td>
                          <td className="px-5 py-3.5 text-gray-400">{d.registrar}</td>
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-2">
                              {expiring && <AlertTriangle className={cn("w-3.5 h-3.5", expired ? "text-red-400" : "text-amber-400")} />}
                              <div>
                                <p className={cn("text-sm", expired ? "text-red-400" : expiring ? "text-amber-400" : "text-gray-300")}>
                          ...
                          <td className="px-5 py-3.5">
                            <span className={cn("text-xs font-medium px-2 py-0.5 rounded-full",
                              d.auto_renew ? "text-emerald-400 bg-emerald-400/10" : "text-gray-500 bg-gray-500/10")}>
                              {d.auto_renew ? "Ativo" : "Inativo"}
                            </span>
                          </td>
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => openEdit(d)} className="p-1.5 text-gray-500 hover:text-purple-400 hover:bg-purple-400/10 rounded transition-colors">
                                <Edit2 className="w-3.5 h-3.5" />
                              </button>
                              <button onClick={() => setDeleteId(d.id)} className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-red-400/10 rounded transition-colors">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-white font-semibold">{editing ? "Editar Domínio" : "Novo Domínio"}</h2>
              ...
              <div className="space-y-4">
                ...
                {(["domain","registrar"] as const).map((field) => (
                  <div key={field}>
                    <label className="text-gray-400 text-xs font-medium mb-1.5 block capitalize">{field === "domain" ? "Domínio" : "Registrar"}</label>
                    <Input value={form[field]} onChange={(e) => setForm((f) => ({ ...f, [field]: e.target.value }))}
                      className="bg-white/5 border-purple-900/40 text-white h-9" />
                  </div>
                ))}
                <div>
                  <label className="text-gray-400 text-xs font-medium mb-1.5 block">Data de Vencimento</label>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <p className="text-gray-400 text-sm mb-5">Deseja excluir este domínio?</p>
